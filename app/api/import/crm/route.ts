...
export async function POST(req: Request) {
  ...
  // Try new format (contacts.json at root), then legacy (data/contacts.json)
  const contactsRaw = zipped["contacts.json"] ?? zipped["data/contacts.json"];
  if (!contactsRaw) {
    return NextResponse.json(
      { error: "Not a valid CRM export: no contacts.json found" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const eventsRaw = zipped["events.json"] ?? zipped["data/events.json"];
    events = eventsRaw ? JSON.parse(strFromU8(eventsRaw)) : [];
    const tagsRaw = zipped["tags.json"] ?? zipped["data/tags.json"];
    tags = tagsRaw ? JSON.parse(strFromU8(tagsRaw)) : [];
  } catch {
